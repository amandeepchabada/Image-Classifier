...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = F.max_pool2d(self.conv2(F.relu(self.conv1(x))),(2,2))
        x = F.max_pool2d(F.relu(self.conv4(F.relu(self.conv3(F.relu(x)))),(2,2)))
        x = torch.flatten(x,1)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
class CIFAR3(Dataset):
    ...
    def __init__(self,split="train",transform=None):
      if split=="train":
        with open("/Users/amandeepchabada/Desktop/AI-535/A3/cifar10_hst_train", 'rb') as fo:
          self.data = pickle.load(fo) 
      elif split=="val":
        with open("/Users/amandeepchabada/Desktop/AI-535/A3/cifar10_hst_val", 'rb') as fo:
          self.data = pickle.load(fo)
      else:
        with open("/Users/amandeepchabada/Desktop/AI-535/A3/cifar10_hst_test", 'rb') as fo:
          self.data = pickle.load(fo)
